...
void scanner::scan_string()
{
	...
	//string initial_string = ""; //initialize string value
	char string_character;
	int sDigit_character;
	int quote_count = 0;  //count the number of quotes

	//get_char();

	bool end_loop = false;

	//double quote turn into single quote, double quote = its own symbol string

	//if ((next_char == '"') and (following_char == '"') and ()) //check for "" before while loop to ensure that it stays a string



	//received help from Mead and Lecture 13 Slides for helping write the scanner while loop below
	while (end_loop == false){
		current_string_value += char(next_char);
		get_char();
		if ((next_char == '"') and (following_char() != '"')){		//check for the current character as a quote and the next not being a quote, ending the code
			current_symbol = new symbol(symbol::string_sym);
			end_loop = true;
		}

		if ((next_char == '"') and (following_char() == '"')){		//check for a double quote
			get_char();
		}

		if (((pos_on_line + 1) == input_buffer.length()) and (following_char() != '"')){		//check to make sure the buffer doesn't overflow before the last quote for the string appears
	...
	//this while loop creates the string itself from start to finish
	//this while loop is based on the finite state machine for strings we created
	/*while((!eof_flag) and (!eoln_flag)) {			//(next_char != ' ') and 
		if(next_char == '"') {		//if the next character is a quote, then the quote can lead to either a letter, another quote, or a white space (which ends the string)
			current_string_value += char(next_char);
			quote_count += 1;
			get_char();
			switch(next_char) {	//determining what to add onto the string
			  	case ('"'): 	
				current_string_value += char(next_char);		//adds a second quote
				quote_count += 1;
				case(' '):
					current_string_value += char(next_char);		//end of string bc " is followed by a whitespace
					if (quote_count % 2 == 0){
						current_symbol = new symbol(symbol::string_sym);			//declare symbol for identifier 
						//current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for identifier
						//current_token->set_string_value(current_string_value);		//set string
						//current_token->get_string_value(); 		//get string 
						//break;
					}
					else{
						error->flag(current_line_number, current_pos_on_line, 125);			//if there is an odd number of quotes, such as " or """, then an error will run saying the string is incomplete
						break;
					}
				}
			  ...
			  /*case(' '):
				//if (following_char() == ' ') {			//end of string bc " is followed by a whitespace
				initial_string += next_char;		//end of string bc " is followed by a whitespace
				if (quote_count % 2 == 0){
					current_symbol = new symbol(symbol::string_sym);			//declare symbol for identifier 
					current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for identifier
					current_token->set_string_value(current_string_value);		//set string
				...
				/*else {
					//initial_string += next_char;
					error->flag(current_line_number, current_pos_on_line, 1);		//default, if it doesn't follow a specific rule corresponding with an error above or doesn't follow the rules in general, then it will output saying the string is not as expected
					break;
				}
				
			}

			if (next_char == string_character) {
			    	//initial_string += next_char;
			  		/*if (following_char() == ' ' && quote_count % 2 != 0) {
						error->flag(current_line_number, current_pos_on_line, 125);			//if there are any amount of quotes before the string and no quote proceeding the string, then an error will run saying the string is incomplete	
						//break;
					}
					else {
						get_char();
		...
		if (next_char == eoln_flag) {
			get_char();
			eoln_flag = true;
			//get_char();
			//error->flag(current_line_number, current_pos_on_line, 60);		//this line says if there is an end of line before the string finishes, return an error that says string must be terminated before end of line
			break;
		}

		if (next_char == eof_flag) {
			get_char();
			eof_flag = true;
			//error->flag(current_line_number, current_pos_on_line, 124);			//this line says if there is an end of file before the string finishes, return an error that says string must be terminated before end of file
			break;
		}

		else {		//if the string starts with anything other than an ", then it is not a string, and will say that a string is expected
			//current_string_value += next_char;
			error->flag(current_line_number, current_pos_on_line, 1);		//default, if it doesn't follow a specific rule corresponding with an error above or doesn't follow the rules in general, then it will output saying the string is not as expected
			break;
		}
...
void scanner::scan_digit()
{
	...
	//int initial_string; //initialize string valuessss
	int digit_character;
	char identifier;
	int subNegSymCounter = 0;
	int digit_character_counter = 0;
	bool realNum = false;
	bool identifier_Yes = false;
	bool range_yes = false;

	get_char();


	while((next_char != ' ') and (!eof_flag) and (!eoln_flag)){
		if(next_char == digit_character) {
			digit_character_counter += 1;
			current_integer_value += next_char;
			get_char();
			switch(next_char) {
				case('.'):		//what to do if there is a decimal after an int (creating a digit)
					current_integer_value += next_char;
					if (following_char() == digit_character){
						current_integer_value += next_char;
						realNum = true;		//the value is now a real number so when the symbol and token are created out of the while loop, it will be a real number 
						//current_symbol = new symbol(symbol::real_num);		//makes the symbol of a digit a real number
					}
					if (following_char() == ' '){
						error->flag(current_line_number, current_pos_on_line, 2);		//should be checking 3.
						break;
					}
				
					/*if (following_char() == '.'){
						initial_string += next_char;
						if (following_char() == digit_character)			//what to do if there is an integer after the range
							initial_string += next_char;

						if (following_char() == '-')			//what to do if there is a '-' symbol after the range
							initial_string += next_char;

						else {
							error->flag(current_line_number, current_pos_on_line, 84);		//if there is any other symbol after a range sign that is not an integer or negative symbol, an error will be returned saying an integer, real, or string expression expected
							break;
						}
					}*/

					else {
						error->flag(current_line_number, current_pos_on_line, 2);		//if there is any other symbol after a decimal that is not an integer, an error will be returned saying an integer is expected
						break;
					}

				case('+'):		//what to do if there is an addition sign after an int
					current_integer_value += next_char;
					if (following_char() == digit_character)
						current_integer_value += next_char;

					if (following_char() == '-'){
						current_integer_value += next_char; 		//allows for adding a negative value
						subNegSymCounter += 1;
					}

					else {
						error->flag(current_line_number, current_pos_on_line, 3);		//if there is any other symbol after an addition that is not an integer, an error will be returned saying an integer is expected
						break;
					}

				case('*'):		//what to do if there is a multiplication sign after an int
					current_integer_value += next_char;
					if (following_char() == digit_character)
						current_integer_value += next_char;

					if (following_char() == '-'){
						current_integer_value += next_char; 		//allows for multiplying a negative value
					}

					else {
						error->flag(current_line_number, current_pos_on_line, 3);		//if there is any other symbol after a multiplication sign that is not an integer, an error will be returned saying an integer is expected
						break;
					}

				case('/'):		//what to do if there is a divided sign after an int 	          
					current_integer_value += next_char;
					if (following_char() == digit_character)
						current_integer_value += next_char;

					if (following_char() == '-')
						current_integer_value += next_char; 		//allows for dividing a negative value

					else {
						error->flag(current_line_number, current_pos_on_line, 3);		//if there is any other symbol after a divison sign that is not an integer, an error will be returned saying an integer is expected
						break;
					}

				case('-'):		//what to do if there is a subtraction sign after an int
					current_integer_value += next_char;
					subNegSymCounter += 1;										//this is only a counter for this case so to count the number of '-' to ensure there are no more than two of these characters in a row
					
					if (following_char() == digit_character)				//allows for there to be an integer after a subtraction sign
						current_integer_value += next_char;

					if (following_char() == '-'){									//helps implement the negative for an integer is okay
						subNegSymCounter += 1;						
						if (subNegSymCounter <= 2)					//this is if there is a negative/subtraction sign after another one, then it should be okay (like 7 - -3); two negatives in a row are okay here
							current_integer_value += next_char;
						
						else {										//however, if there are at least three of the '-' symbols in a row, then it will run an error, saying that a correct integer expression was expected
							error->flag(current_line_number, current_pos_on_line, 3);
							break;
						}
					}
						...
						error->flag(current_line_number, current_pos_on_line, 3);		//if there is any other symbol after a subtraction sign that is not an integer, an error will be returned saying an integer is expected
						break;
					}

				case('..'):		//What to do if there is a range
					current_integer_value += next_char;
					if (following_char() == digit_character){			//what to do if there is an integer after the range
						current_integer_value += next_char;
						range_yes = true;
					...
					if ((following_char() == ' ') || (following_char() == eoln_flag)){
						error->flag(current_line_number, current_pos_on_line, 84);		//if there is any other symbol after a range sign that is not an integer or negative symbol, an error will be returned saying an integer, real, or string expression expected
						break;
					}
				...
				case(' '):		//what to do if there is a whitespace
					current_integer_value += next_char;
					break;
					//current_symbol = new symbol(symbol::integer_sym);		//ends the integer symbol when there is a white space, as this indicates the integer is complete
					//break;
    				
				case('e'):		//what to do if there is an exponent - error 67
				case('E'):
					current_integer_value += next_char;
					if (following_char() == digit_character)
						current_integer_value += next_char;
					...
					if (following_char() == '-'){
						error->flag(current_line_number, current_pos_on_line, 67);		//it will return an error saying that a negative exponent is not allowed
						break;
					}

					//if ((following_char() == '-') and (realNum == true)){
					//	current_integer_value += next_char;
					//}

					else {
						error->flag(current_line_number, current_pos_on_line, 84);		//if there is any other symbol after an addition that is not an integer, an error will be returned saying an integer, real, or string expression expected
						break;
					}
		
			

				case('**'):		//what to do if there is an exponent
					current_integer_value += next_char;
					if (following_char() == digit_character)
						current_integer_value += next_char;
					...
					if (following_char() == '-'){
						...
						error->flag(current_line_number, current_pos_on_line, 18);		//if there is any other symbol after an addition that is not an integer, an error will be returned saying an integer, real, or string expression expected
						break;
					}
				

				default:
					if (next_char == identifier) {			//evaulate if the next_char is an identifier 
						current_integer_value += next_char;
						identifier_Yes = true;			//the value is now an identifier so when the symbol and token are created out of the while loop, it will be an identifier 
						//error->flag(current_line_number, current_pos_on_line, 126);			//the integer cannot include an identifier
						break;
						}
					
					/*if (next_char == digit_character) {	//creates an integer that is creater than one digit
						current_integer_value += next_char;
						digit_character_counter += 1;*/

						if (digit_character_counter > 10) {			//this if statement causes an error message to run if the integer is too big, which in this case is over 10 digits long
					...
					/*else {
						//initial_string += next_char;
						error->flag(current_line_number, current_pos_on_line, 3);		//default, if it doesn't follow a specific rule corresponding with an error above or doesn't follow the rules in general, then it will output saying the integer is not as expected
						break;
					}*/
				}
		...
		/*if ((realNum == false) and (next_char == 'e' || 'E')) {
			current_integer_value += next_char;
			//get_char();
			if (following_char() == '-'){
				error->flag(current_line_number, current_pos_on_line, 67);		//it will return an error saying that a negative exponent is not allowed
		...
		if ((realNum == true) and (next_char == 'e' || 'E')) {
			current_integer_value += next_char;
			get_char();
			if (next_char == '-'){
				current_integer_value += next_char;	
		...
		if(next_char == 'e' || 'E') {				//test for not allowing negative errors afterwards
			current_integer_value += next_char;
			//get_char();
			if (following_char() == '-'){
				error->flag(current_line_number, current_pos_on_line, 67);		//it will return an error saying that a negative exponent is not allowed
		...
		/*if (next_char == eoln_flag) {
			//eoln_flag = true;
			//error->flag(current_line_number, current_pos_on_line, 127);		//this line says if there is an end of line before the integer finishes, return an error that says integer must be terminated before end of line
			//break;
		}

		if (next_char == eof_flag) {
			//eof_flag = true;
			//error->flag(current_line_number, current_pos_on_line, 128);			//this line says if there is an end of file before the integer finishes, return an error that says integer must be terminated before end of file
			//break;
		}

		/*else {		//if the integer starts with anything other than an - or an int, then it is not a string, and will say that a string is expected
			error->flag(current_line_number, current_pos_on_line, 9);		//default, if it doesn't follow a specific rule corresponding with an error above or doesn't follow the rules in general, then it will output saying the integer is not as expected
			//break;
		}*/
	...
	if (realNum == true){
		current_symbol = new symbol(symbol::real_sym);			//declare symbol for real number
		current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for real number
		//current_token->set_real_value(current_real_value); 		//set real value
		//current_token->get_real_value(); 		//get real value        

	}
	if (identifier_Yes = true){
		current_symbol = new symbol(symbol::identifier);		//declare symbol for identifier
		current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for identifier
		//current_token->set_identifier_value(current_identifier_name);		//set identifier value
		//current_token->get_identifier_value();		//get identifier value 
	}
	if (range_yes = true){
		current_symbol = new symbol(symbol::range_sym);		//declare symbol for identifier
		current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for identifier
		//current_token->set_identifier_value(current_);		//set identifier value
		//current_token->get_identifier_value();		//get identifier value 
	}
	else {
		current_symbol = new symbol(symbol::integer_sym);			//declare symbol for integer number
		current_token = new token(current_symbol, current_line_number, current_pos_on_line);		//declare token for integer number
		current_token->set_integer_value(current_integer_value); 		//set integer value
		current_token->get_integer_value(); 		//get integer value
	}
